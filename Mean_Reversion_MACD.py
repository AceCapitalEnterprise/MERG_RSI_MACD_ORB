...
time_2 = t(15, 25)  # 3:01 PM IST -> 9:31 AM UTC
order = 0
order2 = 0
expiry = '2024-10-24'
...
while True:
    now = datetime.now()
    if t(9, 30)<t(datetime.now().time().hour, datetime.now().time().minute)<t(15, 25) :
        today = datetime.now().strftime("%Y-%m-%d")
        yesterday = (datetime.now() - timedelta(days=5)).strftime("%Y-%m-%d")
        
        
        if order == 0 and now.second == 0 and t(datetime.now().time().hour, datetime.now().time().minute)<t(14, 30) :
            SL = 0
            initial_point = 0
            for j in range(0, 5):
                try:
